...
def round_num(n: float, decimal: int = 2) -> Decimal:
    n: Decimal = Decimal(n)
    return n.to_integral() if n == n.to_integral() else round(n.normalize(), decimal)


def numerize(n: int | float, decimal: int = 2) -> str:
    # 60 sufixes
    sufixes: list[str] = ["", "K", "M", "B", "T", "Qa", "Qu", "S", "Oc", "No",
    ...
    sci_expr: list[float] = [1e0, 1e3, 1e6, 1e9, 1e12, 1e15, 1e18, 1e21, 1e24, 1e27,
    ...
    for x in range(len(sci_expr)):
        try:
            if n >= sci_expr[x] and n < sci_expr[x+1]:
                sufix = sufixes[x]
                num: str = str(
                    round_num(n/sci_expr[x], decimal)) if n >= 1e3 else str(n)
                return num + sufix if minus_buff > 0 else f"-{num}{sufix}"
        except IndexError:
